...
public class DMTXLoader {
    ...
    public static void loadLibrary() throws RuntimeException {
        try {
            ...
            String dynamicLibrary = "/native/";
            String jniLibrary = "/jni/";

            if (os.contains("win")) {
                dynamicLibrary += "win/libdmtx.dll";
                throw new UnsupportedOperationException("JNI still missing for OS: " + os);
            } else if (os.contains("linux")) {
                dynamicLibrary += "linux/libdmtx.so";
                jniLibrary += "linux/libdmtx.so";
            } else {
                throw new UnsupportedOperationException("Unsupported OS: " + os);
            }

            List<File> tempFiles = new ArrayList<>();
            for (String library : List.of(dynamicLibrary, jniLibrary)) {
                // Load the library as a resource
                InputStream in = DMTXLoader.class.getResourceAsStream(library);
                if (in == null) {
